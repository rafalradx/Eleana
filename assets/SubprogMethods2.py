...
class SubMethods_02:
    ...
    def do_calc_stk_data(self, name):
        ...
        if y_data1.size == 0:
            ...
            return False
        data_1 = {'x': x_data1, 'y': y_data1, 'z': z_data1, 'name': name1, 'stk_value':z_data1[self.stk_index]}
        ...
        if self.use_second:
            ...
            if self.original_data2.type.lower() != 'single 2D' or is_2D != 2:
                ...
                try:
                    ...
                    data_2 = {'x': x_data2, 'y': y_data2, 'z': z_data2, 'name': name2, 'stk_value':z_data2[self.stk_index]}
                except:
                    # Skip setting 'stk_value'
                    data_2 = {'x': x_data2, 'y': y_data2, 'z': z_data2, 'name': name2, 'stk_value': ''}
    ...
    def do_calc_single2D(self):
        ...
        if y_data1.size == 0:
            ...
            self.set_mouse_state(state='')
            return False
        data_1 = {'x':x_data1, 'y':y_data1, 'z':z_data1, 'name':name1, 'stk_value':'None'}
        self.data_for_calculations.append(data_1)
        ...
        if self.use_second:
            ...
            if self.original_data2.type.lower() != 'single 2D' or is_2D != 2:
                ...
                # Extract the data in x and y
                x_data2, y_data2 = self.extract_region_xy(x=x_data2, y=y_data2)
                data_2 = {'x': x_data2, 'y': y_data2, 'z':z_data2, 'name': name2, 'stk_value':'None'}
                self.data_for_calculations.append(data_2)
            else:
                Error.show(info='If the first data is 2D, the second must also be 2D, not a stack.', details='')
                return False
    ...
    def set_mouse_state(self, state = ""):
        ''' Set curosr to watch or ready for Main GUI and Main Graph'''
        if self.app:
