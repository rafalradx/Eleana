...
class Grapher:
    def __init__(self, app_instance, eleana_instance):
        ...
        empty_re_y = np.array([])
        empty_im_y = np.array([])
        empty_legend = 'no plot'
        #self.ax.plot(empty_x, empty_re_y, label=empty_legend)
        #self.ax.plot(empty_x, empty_re_y, label=empty_legend)
        #self.ax.plot(empty_x, empty_re_y, label=empty_legend)
    ...
    def plot_graph(self):
        ...
        if len(self.eleana.results_dataset) != 0:
            ...
            if self.eleana.selections['s_dsp'] and is_result_not_none:
                data_for_plot = self.eleana.getDataFromSelection(eleana, 'result')
        ...
        #canvas = FigureCanvasTkAgg(fig, master=self.app.graphFrame)
        #canvas.get_tk_widget().grid(row=0, column=0, sticky="nsew")

        self.canvas.draw()


        #toolbar = NavigationToolbar2Tk(canvas, self.app.graphFrame, pack_toolbar=False)
        #toolbar.update()
