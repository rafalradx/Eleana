...
class Dial(tk.Canvas):
    ...
    def __init__(
        ...
        self.value_text = self.create_text(
            self.__x,
            self.__y + self.__radius + 25,
            text=f"{self.__text_title}{self.value:.1f}",
            font=("Helvetica", 10),
            fill="white"
        )
        ...
        self.scroll_sensitivity = scroll_sensitivity # for wheel
        self.drag_sensitivity = drag_sensitivity   # for drag

    def scroll_command(self, event):
        """
        Endless encoder scroll behavior for mouse wheel.
        """
        ...
        self.value += direction * self.scroll_sensitivity
    ...
    def update_encoder_display(self):
        ...
        self.__colorize(angle=angle, start=0, end=int(angle), color="")

        if hasattr(self, "value_text"):
            self.itemconfig(self.value_text, text=f"{self.__text_title}{self.value:.1f}")
    ...
    def __rotate_needle(self, event, angle=None, skip_command = False) -> None:
        ...
        self.value += delta_angle * self.drag_sensitivity
