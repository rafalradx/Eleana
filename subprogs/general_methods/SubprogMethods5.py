...
class SubMethods_05:
    ...
    ''' Ten fragment tworzy listę instancji tej klasy
        TYLKO DO DEBUGINGU'''
    _instances = []
    def __init__(self, app=None, commandline=False, which = 'first', close_subprogs = False):
        ...
        if app and not self.commandline:
            ...
            self.master = weakref.ref(self.app().mainwindow)
            self.eleana = weakref.ref(self.app().eleana)
            #self.eleana().active_subprog = self
            self.grapher = weakref.ref(self.app().grapher)
            self.update = weakref.ref(self.app().update)
    ...
    def cancel(self, event=None):
        ...
        # Cleanup custom widgets
        self._detach_custom_widgets_commands()
        self._destroy_custom_widgets()
        ...
        self.mainwindow.protocol('WM_DELETE_WINDOW', None)
    ...
    def perform_group_calculations(self):
        ...
        copy_selections = self.eleana().selections.copy()
    ...
    def create_result_stack(self):
        ...
        new_result.x = copy(self.data_for_calculations[0]['x'])
        new_result.y = copy(self.data_for_calculations[0]['y'])
        new_result.z = copy(self.data_for_calculations[0]['z'])
        new_result.name = copy(self.data_for_calculations[0]['name'])
        new_result.complex = copy(self.data_for_calculations[0]['complex'])
        new_result.type = copy(self.data_for_calculations[0]['type'])
        new_result.origin = copy(self.data_for_calculations[0]['origin'])
        new_result.comment = copy(self.data_for_calculations[0]['comment'])
        new_result.parameters = copy(self.data_for_calculations[0]['parameters'])
    ...
    def create_result(self):
        ...
        list_of_results = [obj.name for obj in results_dataset]
        new_result = copy.deepcopy(self.original_data1)
        new_result.x = copy.copy(self.data_for_calculations[0]['x'])
        new_result.y = copy.copy(self.data_for_calculations[0]['y'])
        new_result.z = copy.copy(self.data_for_calculations[0]['z'])
        new_result.name = copy.copy(self.data_for_calculations[0]['name'])
        new_result.complex = copy.copy(self.data_for_calculations[0]['complex'])
        new_result.type = copy.copy(self.data_for_calculations[0]['type'])
        new_result.origin = copy.copy(self.data_for_calculations[0]['origin'])
        new_result.comment = copy.copy(self.data_for_calculations[0]['comment'])
        new_result.parameters = copy.copy(self.data_for_calculations[0]['parameters'])
        ...
        if self.stk_index == -1 and results_dataset:
            ...
            if self.stk_index <= len(results_dataset[index_in_result].y) - 1:
                ...
                results_dataset[index_in_result].x = copy.copy(new_result.x)
                results_dataset[index_in_result].z = copy.copy(new_result.z)
                results_dataset[index_in_result].complex = copy.copy(new_result.complex)
                results_dataset[index_in_result].type = copy.copy(new_result.type)
    ...
    def custom_widget(self, widget):
        ''' Register widget'''
        self.registered_custom_widgets.append(widget)
        return widget

    def _detach_custom_widgets_commands(self):
    ...
    def _destroy_custom_widgets(self):
        for widget in getattr(self, "egistered_custom_widgets", []):
            try:
                ...
                print(f"cannot destroy {widget}: {e}")
